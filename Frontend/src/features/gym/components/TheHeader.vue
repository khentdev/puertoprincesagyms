<template>
  <header
    class="h-18 flex bg-bg-panel items-center px-4 border-b border-border-subtle"
  >
    <div class="flex items-center gap-2">
      <img
        src="../../../../assets/images/logos/BrandLogo.png"
        alt="Puerto Princesa Gym Directory Logo"
        class="size-12 sm:size-16 md:size-20 object-contain"
      />
      <div class="flex flex-col items-start justify-center">
        <h1 class="text-base sm:text-lg font-bold text-text-high-contrast">
          Puerto Princesa Gym Directory
        </h1>
        <p class="text-xs sm:text-sm text-text-low-contrast">
          Find your perfect workout spot in the city
        </p>
      </div>
    </div>
  </header>
</template>

<script lang="ts" setup></script>
